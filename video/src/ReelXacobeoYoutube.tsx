...
/**
 * ReelXacobeo en horizontal, para YouTube (1280x720).
 *
 * Mismo guion, misma locucion y mismos planos que la version vertical: los
 * arranques de frase y la duracion de cada bloque son identicos. Lo que
 * cambia es la composicion de cada escena. En vertical el texto vive en la
 * mitad superior y el grafico en la inferior porque sobra alto para apilarlos;
 * en un lienzo de 720 px de alto ese reparto no cabe.
 *
 * El guion ya evita que una cartela y un grafico coincidan en el mismo
 * bloque (si el bloque lleva grafico, el grafico trae su propio titular), asi
 * que no hay que resolver ese choque: la cartela se queda arriba a la
 * izquierda tal cual, y el grafico se centra solo en los bloques sin cartela.
 * El Calendario, pensado para un lienzo mucho mas alto, se escala para
 * caber sin recortarse.
 */
...
/** Zona del grafico: centrada, sin la reserva de alto que pedia el vertical. */
...
export const ReelXacobeoYoutube: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: brand.forest, fontFamily }}>
      ...
      <Sequence from={f(B[1])} durationInFrames={dur(1)} name="2 · Calendario">
        ...
        />
        <Centro>
          {/* El calendario esta pensado para un lienzo vertical mucho mas
           * alto: sin escalar no cabe en 720 px de alto. */}
      ...
      <Sequence from={f(B[5])} durationInFrames={dur(5)} name="6 · Servicios">
        ...
        />
        <Centro>
          {/* Los cinco bullets, uno bajo otro, tampoco caben a tamano
           * natural en 720 px de alto: se escalan igual que el calendario. */}
...
export const DURACION_REEL_YOUTUBE = f(B[B.length - 1]);
